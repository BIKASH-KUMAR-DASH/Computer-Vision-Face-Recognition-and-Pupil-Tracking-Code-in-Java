...
public class PupilTracer{
	...
	public static void main(String[] args) {		
		...
		if (videoDevice.isOpened()) {
			while (true) {		
				...
				pupilTracker(frameCapture, cascadeFaceClassifier, cascadeEyeClassifier);
	...
	private static void pupilTracker(Mat frameCapture, CascadeClassifier cascadeFaceClassifier, CascadeClassifier cascadeEyeClassifier) {
		
		
		//Code Snippet for Face Detection
		//faceDetection function  performs FACE Detection
		//faceDetection(frameCapture, cascadeFaceClassifier);
		...
		for (Rect rect : faces.toArray()) {
			...
			pupilSegmentation(frameCapture, faceImage, cascadeEyeClassifier, rect);
	...
	private static void pupilSegmentation(Mat frameCapture, Mat faceImage, CascadeClassifier cascadeEyeClassifier, Rect rect) {
		
		MatOfRect eyes = new MatOfRect();
		cascadeEyeClassifier.detectMultiScale(faceImage, eyes);		
		...
		for (Rect rect1 : eyes.toArray()) {
			...
			Imgproc.putText(frameCapture, "Eye", new Point(rect1.x + rect.x,rect1.y + rect.y), 1, 2, new Scalar(0,0,255));				
			
			Imgproc.rectangle(frameCapture, new Point(rect1.x + rect.x, rect1.y + rect.y), new Point(rect1.x + rect.x + rect1.width, rect1.y + rect.y  + rect1.height),	new Scalar(200, 200, 100),2);
		
			rectEyeCrop = new Rect(rect1.x , rect1.y , rect1.width, rect1.height);
		
			
			
			//Code Snippet for Pupil Tracking
			//cropping the eye region first
			Mat im1= new Mat(faceImage,rectEyeCrop);		
			...
			for (int i=0; i < list.size(); i++){
				pt.x = (int)list.get(i).pt.x + rect1.x + rect.x; // Finding Pupil's x-coordinates
			    pt.y = (int)list.get(i).pt.y + rect1.y + rect.y; // Finding Pupil's y-coordinates
